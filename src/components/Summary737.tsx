// Summary737.tsx (no Tailwind - uses summary-glass.css)
...
function Sparkline({ values = [] }: { values?: number[] }) {
  ...
  const points = values
    ...
    .join(" ");
  return (
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" width="100%" height="36" aria-hidden>
      <defs>
        <linearGradient id="g737" x1="0" x2="1" y1="0" y2="0">
...
export default function Summary737({ month }: { month?: string }) {
  ...
  useEffect(() => {
    ...
    async function fetchSummary() {
      ...
      try {
        ...
        console.error("737 summary fetch error", err);
